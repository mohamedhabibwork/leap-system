import { pgTable, bigserial, bigint, uuid, varchar, text, timestamp, boolean, index, integer } from 'drizzle-orm/pg-core';
...
export const users = pgTable('users', {
  ...
  index('users_uuid_idx').on(table.uuid),
  index('users_email_idx').on(table.email),
  index('users_username_idx').on(table.username),
  index('users_role_id_idx').on(table.roleId),
  index('users_status_id_idx').on(table.statusId),
  ...
  index('users_keycloak_user_id_idx').on(table.keycloakUserId),
  index('users_subscription_status_idx').on(table.subscriptionStatus),
  index('users_current_subscription_id_idx').on(table.currentSubscriptionId),
]));
...
export const userRoles = pgTable('user_roles', {
  ...
  index('user_roles_user_role_unique_idx').on(table.userId, table.roleId),
]));
...
export const rolePermissions = pgTable('role_permissions', {
  ...
  index('role_permissions_role_permission_unique_idx').on(table.roleId, table.permissionId),
]));
